(function() {
    ...
    var simAlertError = {
        template: '<div class="alerts" ng-cloak="" role="alert">' +
                        ...
                        '</div>' +
                  '</div>',
        controller: simAlertErrorController
    };

    angular
        .module('simlifeApp')
        .component('simAlertError', simAlertError);

    simAlertErrorController.$inject = ['$scope', 'AlertService', '$rootScope'];

    function simAlertErrorController ($scope, AlertService, $rootScope) {
        var vm = this;
        ...
        var cleanHttpErrorListener = $rootScope.$on('simlifeApp.httpError', function (event, httpResponse) {
            var i;
